...
write_csv(tempdata0, here("docs/plantTraitsSubset.csv"))
```
...
```{r}
dir.create(here('docs/elements'), showWarnings = FALSE)
...
icons <- data.frame(urls=paste0(urls, c("check_circle","cancel"),"/default/48px.svg"),
                    svgs = here('docs/elements', paste0(c("1","0"),".svg")),
...
image_write(image_colorize(present, 100, 'white'), 
            path = here('docs/elements',"NA.svg"), format = "svg")
...
```{r}
icons <- data.frame(icons = list.files(here('docs/elements'), full.names = TRUE, pattern = '.svg')) %>%
...
```{r}
icons <- data.frame(icons = list.files(here('docs/elements'), full.names = TRUE, 
                                       pattern = '.svg')) %>%
...
```{r, code = readLines(here('docs/app.R')), eval = FALSE}
```
